...
public abstract class AbstractTallCeilingPlantBlock extends PlantBlock {
    public static final EnumProperty<DoubleBlockHalf> HALF = Properties.DOUBLE_BLOCK_HALF;

    public AbstractTallCeilingPlantBlock(Settings settings) {
        super(settings);
    ...
    public void onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        if (!world.isClient) {
            if (player.isCreative()) {
                AbstractTallCeilingPlantBlock.onBreakInCreative(world, pos, state, player);
            } else {
                AbstractTallCeilingPlantBlock.dropStacks(state, world, pos, null, player, player.getMainHandStack());
            }
        }
